  ...
  for (const auto & jointEntity : jointEntities) {
    ...
    switch (jointType->Data()) {
      ...
      case sdf::JointType::CONTINUOUS:
        {
          std::cout << "CONTINOUS JOINT HEARD" << std::endl << std::endl;
        }
  ...
  ignition::gazebo::EventManager &)
{
  ...
  if (!rclcpp::ok()) {
    ...
    std::string node_name = "ignition_ros_control_mark";
    if (!controllerManagerPrefixNodeName.empty()) {
      node_name = controllerManagerPrefixNodeName + "_" + node_name;
    }
    this->dataPtr->node_ = rclcpp::Node::make_shared(node_name);
    // this->dataPtr->node_->declare_parameter("Mark", 10);
    // this->dataPtr->node_->create_subscription("/mark_cmd_vel", )
    // this->dataPtr->node = ignition::transport::Node node;
  }
  // std::string topic = "/mark_cmd_vel";
  // this->dataPtr->node_->create_subscription<ignition::msgs::Twist>(topic, rclcpp::QoS(10),
  //    std::bind(&IgnitionROS2ControlPlugin::OnCmdVel, this, std::placeholders::_1 ))

  // subscription = create_subscription<turtlesim::msg::Pose>("turtle1/pose", 10,
  //       std::bind(&broadcaster::callback, this, std::placeholders::_1 ) );
  // std::cout << this->dataPtr->node.Subscribe(topic, &IgnitionROS2ControlPlugin::OnCmdVel, this->dataPtr.get()) << std::endl;
  ...
  if (enabledJoints.size() == 0) {
    ...
    std::cout << "WE HAVE JOINTS AVAILABLE" << std::endl;
  }
...
void IgnitionROS2ControlPlugin::OnCmdVel(const ignition::msgs::Twist &_msg){
  targetVel = _msg;
}
