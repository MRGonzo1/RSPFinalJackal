...
public:
  ...
  /// \brief An array per IMU with 4 orientation, 3 angular velocity and 3 linear acceleration
  std::array<double, 4> vel_cmd_data_;

  /// \brief callback to get the IMU topic values
  void VelCmds(const ignition::msgs::Twist & _msg);
};

void VelCmd::VelCmds(const ignition::msgs::Twist & _msg)
{

  // std::cout << "HEARD" << std::endl;
  this->velocity_cmd = _msg;

  // std::cout << this->velocity_cmd.linear().x() << "::" << this->velocity_cmd.angular().x()<< std::endl;
  // double temp = _msg.linear().x();
  // this->velocity_cmd.linear().set_x(temp);
  // geometry_msgs::msg::Twist command;
  
  this->vel_cmd_data_[0] = 2*this->velocity_cmd.linear().x() - this->velocity_cmd.angular().z();
  this->vel_cmd_data_[1] = 2*this->velocity_cmd.linear().x() - this->velocity_cmd.angular().z();
  this->vel_cmd_data_[2] = 2*this->velocity_cmd.linear().x() + this->velocity_cmd.angular().z();
  this->vel_cmd_data_[3] = 2*this->velocity_cmd.linear().x() + this->velocity_cmd.angular().z();
...
public:
  ...
  /// \brief vector with the velocity commands .
  std::vector<std::shared_ptr<VelCmd>> vels_;
  ...
  const hardware_interface::HardwareInfo & hardware_info)
{
  ...
  this->dataPtr->ecm->Each<ignition::gazebo::components::Imu,
    ...
    auto velData = std::make_shared<VelCmd>();
    velData->name = "Mark";
    velData->topicName = "/vel_cmd";
    this->dataPtr->vels_.push_back(velData); 
}
...
hardware_interface::return_type IgnitionSystem::read()
{
  ...
  std::string vel_topic = "/diff_drive_base_controller/cmd_vel_unstamped";
  // std::string vel_topic = "/cmd_vel";
  this->dataPtr->node.Subscribe( vel_topic, &VelCmd::VelCmds, this->dataPtr->vels_[0].get());
          // this->dataPtr->imus_[0].get());
